...
*/
		...
		if($member_info["mtype"] == "2" || $member_info["mtype"] == "4" || $member_info["mtype"] == "7") {
			...
			if(date("Y-m-d H:i:s") > $check_date && $row["sdate"] > date("Y-m-d H:i:s")) {
				$button = "투자하기";
				$url	= "javascript: invest_submit();";
			}
		}
...
//print_r($member_info);
	...
	<script type="text/javascript">
		...
		function reply_del(rnum, depth) {

			var mode = "<?=XOREncode('reply_del');?>";
			var goodsno = "<?=$_POST['num'];?>";

			if(confirm('삭제하시겠습니까?') == true) {
				$.ajax({
					type	:	"POST",
					data	:	{"rnum":rnum, "depth":depth, "mode":mode, "goodsno":goodsno},
					url		:	"/inc/state.php",
					success	:	function(data) {
						$('#reply_div').html(data);
					}
	...
	<script>
		$(function() {
			...
			//# 댓글 작성
			$('#reply_div').on('click', ".reply2_button", function() {
				var reply_form = $(this).closest('form');
				var content = reply_form.find('textarea');
			...
			//# 답글 작성
			var comments = $(".cont-comments"); 
			// 댓글 - 쓰기 클릭시 댓글 입력창 등장 
			comments.on("click", ".post .btn-reply", function() { 
				$(this).closest(".post").toggleClass("on"); 
			}); 
		});
	</script>
    ...
    <section class="estate">
      <div class="container">
			   ...
			   <!-- 이미지 -->
				<div class="invest-img">
					...
					<?php
						if($row["auto_chk"] == "Y") {
					?>
					<!-- 190619 자동 투자가 가능할경우 추가-->
					<div class="autoinv">
						<span class="txt">자동투자상품</span>
					</div>
					<?php
						}
					?>
					...
					<!-- 20191226 누리세이프 -->
					<div class="safe-mark">
						<img src="https://nurifunding.co.kr/img/nurisafe/safe_mark.png" alt="누리세이프마크">
					</div>
					<?php
						}
					?>
					...
					<div class="row invest-input-money-box">
						<div class="invest-input-money">
						   <p class="nr-box invest-input-box br4">
								<input type="number" value="" name="invest_price" onkeyup="javascript: checktype('number', 'invest_price')" class="nr-text invest-input" placeholder="0"/>
								<span>만원</span>
						   </p>
		...
		?>
		<!-- 20191226 누리세이프 -->
		<div class="com-box bnBar-wrap nurisafeBn">
			<h2 class="hidden">채권의 연체 및 투자 원금 손실위험에서 투자자를 보호하는 누리세이프 플랜으로 안심하고 투자하세요!</h2>
			<figure>
				<a href="/member/safe.php">
					<img src="https://nurifunding.co.kr/img/banner/nurisafe/s_bnr_safe.jpg" alt="누리세이프 플랜" class="hidden-xs">
					<img src="https://nurifunding.co.kr/img/banner/nurisafe/s_bnr_safe_m.jpg" alt="누리세이프 플랜" class="hidden-md">
				</a>
			</figure>
		</div>
		<?php
        ...
        <div class="row com-box est-wrap">
          <div class="est-box est-com nth-5 br4 col-xs-12">
			...
			<?php if($row['mtype'] == 2 && $row["miri"] == "Y") { ?>
			<figure class="product-miripay hidden-xs">
          		<img src="//nurifunding.co.kr/img/invest/product/miripay.jpg" alt="선정산 상품 방식">
          ...
          <div class="est-box est-com nth-7 br4 col-xs-12">
			...
			<div class="rEstate-per row com-box br4">
				...
				var config = {
					type: 'line',
					data: {
						labels: ["<?=date('Y.12', strtotime('-3 year'))?>", "<?=date('Y.12', strtotime('-2 year'))?>", "<?=date('Y.12', strtotime('-1 year'))?>", "현재"],
						datasets: [{
							label: "현재시세",
        ...
        <div class="row com-box est-wrap">
          <div class="est-box est-com nth-9 br4 col-xs-12">
			...
			<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=2dcb223f25acdf0991cc6d0d55a45d17&libraries=services"></script>
			<script>
			...
			// 주소로 좌표를 검색합니다
			geocoder.addressSearch('<?=$map_info[1]?>', function(result, status) {
				...
				// 정상적으로 검색이 완료됐으면 
				 if (status === daum.maps.services.Status.OK) {
					...
					var infowindow = new daum.maps.InfoWindow({
						content: '<div style="width:150px;text-align:center;padding:6px 0;font-size:12px;"><?=$row["name"]?></div>'
					});
          ...
          <div class="est-box est-com nth-16 br4 col-xs-12">
            <h3 class="est-com-h3"><span class="fc-blue" id="return">7.</span> 대출자 정보</h3>
					...
					<div class="est-box est-com nth-12 br4 col-xs-12 com-type2">
						<h4 class="est-com-h3"><?=($row["mtype"] == 2) ? '기업' : '개인'?>정보</h4>
		...
		<!--댓글 기능-->
		<div class="row com-box nth-10 cont-comments">
			<div class="est-com br4 col-xs-12" id="reply_div">
				<?php 
					$goodsno = $_POST["num"];
					include_once("/home/ebizdev/web-home/nurifunding.co.kr/www/web-home/invest/reply.php");
				?>
			</div>
		</div>
	   <!--//.cont-contents-->
	...
	<div class="rBn-box-m" id="rBn-box-m">
		<!-- <a href="https://www.nurifunding.co.kr/notice/view.php?var=HUY=&page=2"> -->
		<!-- <a href="http://pf.kakao.com/_xnjkKC" target="_blank">
			<img src="https://www.nurifunding.co.kr/img/banner/banner_katalk_pc.png" alt="카카오톡 플러스 친구" />
		</a> -->
		<!--
